...
void AffTransform::TurnX(Object *obj, double angle) {
    if (angle != 0.0) {
        angle *= RAD_TO_GRAD;
        for (int i = 0; i < obj->getCountVertexes() * 3; i += 3) {
            double temp_y = obj->vertexes_[i + 1];
            double temp_z = obj->vertexes_[i + 2];
            obj->vertexes_[i + 1] = temp_y * cos(angle) + temp_z * sin(angle);
            obj->vertexes_[i + 2] = -temp_y * sin(angle) + temp_z * cos(angle);
        }
    }
}
...
void AffTransform::TurnY(Object *obj, double angle) {
    if (angle != 0.0) {
        angle *= -RAD_TO_GRAD;
        for (int i = 0; i < obj->getCountVertexes() * 3; i += 3) {
            double temp_x = obj->vertexes_[i];
            double temp_z = obj->vertexes_[i + 2];
            obj->vertexes_[i] = temp_x * cos(angle) + temp_z * sin(angle);
            obj->vertexes_[i + 2] = -temp_x * sin(angle) + temp_z * cos(angle);
        }
    }
}
...
void AffTransform::TurnZ(Object *obj, double angle) {
    if (angle != 0.0) {
        ...
        for (int i = 0; i < obj->getCountVertexes() * 3; i += 3) {
            double temp_x = obj->vertexes_[i];
            double temp_y = obj->vertexes_[i + 1];
            obj->vertexes_[i] = temp_x * cos(angle) + temp_y * sin(angle);
            obj->vertexes_[i + 1] = -temp_x * sin(angle) + temp_y * cos(angle);
        }
    }
}
